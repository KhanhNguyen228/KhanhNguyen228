...
SELECT
	...
	AVG(LightlyActiveMinutes) AS avg_lightly_activehour,
	AVG(SedentaryMinutes) AS avg_sedentary_hour,
	ROUND((SELECT
...
-- Determine the average steps each user recorded
SELECT
	DISTINCT Id,
	AVG(TotalSteps) OVER (PARTITION BY Id) AS avg_steps
FROM
	daily_activity_cleaned
